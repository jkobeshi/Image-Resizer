...
void find_minimal_vertical_seam(const Matrix* cost, int seam[]) {
    int small_col = Matrix_column_of_min_value_in_row(cost, 
        Matrix_height(cost) - 1, 0, Matrix_width(cost) - 1);
    seam[Matrix_height(cost) - 1] = small_col;
    for (int i = Matrix_height(cost) - 2; i >= 0; i--) {
        if (small_col == 0) {
            seam[i] = Matrix_column_of_min_value_in_row(cost,
                i, small_col, small_col + 2);
        }
        else if (small_col == Matrix_width(cost) - 1) {
            seam[i] = Matrix_column_of_min_value_in_row(cost, 
                i, small_col - 1, small_col + 1);
        }
        else {
            seam[i] = Matrix_column_of_min_value_in_row(cost, 
                i, small_col - 1, small_col + 2);
        }
        small_col = seam[i];
    }
}
...
void seam_carve_width(Image *img, int newWidth) {
    ...
    while (Image_width(img) != newWidth) {
        Matrix* enrgy = new Matrix;
        Matrix* cst = new Matrix;
        int seam[MAX_MATRIX_HEIGHT];
        compute_energy_matrix(img, enrgy);
        compute_vertical_cost_matrix(enrgy, cst);
        find_minimal_vertical_seam(cst, seam);
...
void seam_carve(Image *img, int newWidth, int newHeight) {
    assert(0 < newWidth && newWidth <= Image_width(img));
    assert(0 < newHeight && newHeight <= Image_width(img));
