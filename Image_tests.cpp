...
TEST(test_image_basic) {
	...
	Pixel green = { 0, 255, 0 };
	Image* img = new Image;
	Image_init(img, 10, 11);

	ASSERT_EQUAL(Image_width(img), 10);
	ASSERT_EQUAL(Image_height(img), 11);

	Image_fill(img, red);
	ASSERT_TRUE(Pixel_equal(Image_get_pixel(img, 2, 2), red));

	Image_set_pixel(img, 10, 9, green);
	ASSERT_TRUE(Pixel_equal(Image_get_pixel(img, 10, 9), green));
